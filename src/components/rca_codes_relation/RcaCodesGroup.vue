...
<v-card class="mt-5">
  <v-toolbar color="teal" dark>
          ...
          <v-toolbar-title class="text-xs-center"><h1>{{ rcaGrupaNaslov }}  - RCA Codes</h1></v-toolbar-title>
          <v-spacer></v-spacer>
    ...
    <v-card-title>
      <v-subheader><h2>{{ rcaSubGrupaNaslov}}</h2></v-subheader>
      <v-spacer></v-spacer>
      <v-text-field
        append-icon="search"
        label="Search"
...
export default {
  ...
  data () {
    ...
    return {
       ...
       /* rca: 'rca-pr',*/
        ...
        search:'',
      headers: [
        { text: 'RCA Code', align: 'left', value: 'defaultCodeId' },
        { text: 'Code Description', align: 'left', value: 'codeDescription' },
        { text: 'Actions', align: 'right', value: 'rcaCodeId', sortable: false }
      ],
      items: [],
      editedIndex: -1,
      editedItem: {
  ...
  computed: {
      formTitle () {
        return this.editedIndex === -1 ? 'New Item' : 'Edit RCA Code'
      }
    },
    ...
    watch: {
      ...
      rca (val) { // on change link param novi axios poziv

        

      const findGroup = this.findObjectByKey(this.rcaGroups, 'routeId', this.rca);

      this.rcaGrupaNaslov = findGroup.group;
      this.rcaSubGrupaNaslov = findGroup.subGroup;
    ...
    created () {
      ...
      const rootcausecodes = this.$store.state.rootCauseCodes;

      this.rcaGroups = rootcausecodes;
    ...
    methods: {
      ...
      initialize () {
        this.items = []
        var vm = this;
        axios.get('/api/RcaCode/'+this.rca, {headers: {'X-Custom-Header': 'foobar'}}).then(
         function(response) {
      ...
      save () {
        if (this.editedIndex > -1) {
          ...
          const formDataPut = {
                ...
                rcaCodeId: this.editedItem.rcaCodeId,
                relatedRootCodeId: this.rcaGrupaNaslov
          ...
          axios.put('/api/RcaCode/'+this.editedItem.rcaCodeId, formDataPut)
                    .then(response => {
